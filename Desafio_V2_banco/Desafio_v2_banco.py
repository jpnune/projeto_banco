...
def criar_usuario(cpf, nome):
    ...
    limpar_tela()
    if validar_data_nascimento(data_nascimento) == False:
...
def depositar(saldo, quantidade_de_operacoes, extrato,/):
    while True:
        try:
    ...
    return saldo, quantidade_de_operacoes, extrato
...
# Fução para limpar a tela das informações anteriores
def limpar_tela():
    print(f'{"\n"*20}') #limpar a tela 
...
def sacar(*, saldo, quantidade_de_operacoes, extrato, quantidide_saque_diario_realizado, VALOR_MAXIMO_POR_SAQUE):
        ...
        if saque_validado == True:
            ...
            limpar_tela() 
            if confirmacao == '1':
                saldo -= float(saque) # Retira o valor do saldo.
                quantidade_de_operacoes += 1 # Soma 1 ao contador "quantidade de operações" para ser usado no extrato.
                ...
                return saldo, quantidade_de_operacoes, extrato, quantidide_saque_diario_realizado
            elif confirmacao == '0':
    ...
    # Refere-se a opção 3 do Menu
        ...
        while True: 
            saldo, quantidade_de_operacoes, extrato, quantidide_saque_diario_realizado = sacar(
                saldo=saldo,
                quantidade_de_operacoes=quantidade_de_operacoes,
                extrato=extrato,
                quantidide_saque_diario_realizado=quantidide_saque_diario_realizado,
                VALOR_MAXIMO_POR_SAQUE= VALOR_MAXIMO_POR_SAQUE)
